...
class SSILoss(nn.Module):
    ...
    def forward(self, pred, target, mask):
        ...
        B,C,H,W = pred.shape
        pred = pred.view(B,-1)
        t_pred = torch.median(pred, dim=1)[0].expand(C * H * W, -1).transpose(1,0)
        s_pred = torch.mean(torch.abs(pred - t_pred), dim=1).expand(C * H * W,-1).transpose(1,0)
    
        pred_aligned = ((pred - t_pred) / s_pred)
        
        target = target.view(B,-1)
        t_target = torch.median(target, dim=1)[0].expand(C * H * W, -1).transpose(1,0)
        s_target = torch.mean(torch.abs(target - t_target), dim=1).expand(C * H * W,-1).transpose(1,0)
    
        target_aligned = ((target - t_target) / s_target)
        
        diff = (pred_aligned - target_aligned) ** 2

        diff = diff.view(-1)
        mask = mask.view(-1)
        
        threshold = np.percentile(diff.cpu().detach().numpy(), 50)
...
def ssl_depth_align_shift_and_scale_train(pred_disp, scale, shift):
    
    B,C,H,W = pred_disp.shape
    pred_disp = pred_disp.view(B,-1)
    
    t_pred = torch.median(pred_disp, dim=1)[0].expand(C * H * W, -1).transpose(1,0)
    s_pred = torch.mean(torch.abs(pred_disp - t_pred), dim=1).expand(C * H * W,-1).transpose(1,0)

    pred_disp_aligned = ((pred_disp - t_pred) / s_pred)
    min_pred_aligned = torch.min(pred_disp_aligned, dim=1)[0].expand(C * H * W, -1).transpose(1,0)

    pred_disp_aligned = (pred_disp_aligned - min_pred_aligned).view(B,C,H,W)
    
    # pred_disp_aligned *= scale
    # pred_disp_aligned += shift
    # pred_disp_aligned = pred_disp_aligned.view(B,C,H,W)
    # pred_disp_aligned[pred_disp_aligned < 1/150] = 1/150
    
    return pred_disp_aligned

def norm_shift_and_scale(disp):
    t = np.median(disp)
    s = np.mean(np.abs(disp - t))
    norm_disp = (disp - t) / s
...
def align_shift_and_scale(gt_disp, pred_disp):
    ...
    pred_disp_aligned[pred_disp_aligned <=0] = 1e-3
